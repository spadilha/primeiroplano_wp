...
class FocusKeywordCount extends AC\Column {
	...
	private function get_focus_keyword( $id ) {
		return get_metadata( $this->get_list_screen()->get_meta_type(), $id, '_yoast_wpseo_focuskw', true );
	}
	...
	public function get_value( $id ) {
		...
		// TODO use SEO title/ post title
		$values = [
			sprintf( ac_helper()->html->tooltip( $this->calculate_occurrence( get_post_field( 'post_title', $id, 'raw' ), $key ), __( 'Title' ) ) ),
			sprintf( ac_helper()->html->tooltip( $this->calculate_occurrence( get_post_field( 'post_content', $id, 'raw' ), $key ), __( 'Content' ) ) ),
		];
